...
function startLightboxSlideshow() {
  ...
  if (currentGallery.length > 0) {
    ...
    // Eğer video ise, otomatik kaydırmayı başlatma. Videonun bitmesini bekle.
    if (isVideo) return;
  }
  ...
  lightboxSlideInterval = setInterval(() => {
    if (currentGallery.length > 0) {
      ...
      showLightbox(currentGallery[currentIndex]);
      resetLightboxSlideshow(); // Otomatik geçişten sonra yeniden başlat ki resimse süre işlesin
    }
  }, SLIDE_INTERVAL);
}
...
// Video bittiğinde otomatik sonraki slayta geç (sadece video ise devreye girer)
if (lightboxVideo) {
  lightboxVideo.addEventListener('ended', () => {
    if (lightboxModal.classList.contains('active') && currentGallery.length > 0) {
      currentIndex = (currentIndex + 1) % currentGallery.length;
      showLightbox(currentGallery[currentIndex]);
...
function updateLightboxThumbnails() {
  ...
  currentGallery.forEach((src, idx) => {
    ...
    if (isVideo) {
      ...
      thumbNode.innerHTML = '▶';
      thumbNode.style.display = 'inline-flex';
      thumbNode.style.alignItems = 'center';
      thumbNode.style.justifyContent = 'center';
      thumbNode.style.background = '#1f3c73'; // Teal
      thumbNode.style.color = '#d4af37'; // Gold
      thumbNode.style.width = '54px';
      thumbNode.style.height = '38px';
      thumbNode.style.margin = '0 0.5rem';
      thumbNode.style.cursor = 'pointer';
      thumbNode.style.borderRadius = '4px';
      thumbNode.style.opacity = idx === currentIndex ? '1' : '0.6';
      if (idx === currentIndex) thumbNode.style.border = '2px solid var(--yellow)';
    } else {
      thumbNode = document.createElement('img');
      thumbNode.src = src;
      thumbNode.className = idx === currentIndex ? 'active' : '';
    }
    ...
    lightboxThumbnails.appendChild(thumbNode);
  });
}
...
function initCounters() {
  ...
  const speed = 100; // Sayma hızı bölme çarpanı
      ...
      if (entry.isIntersecting) {
        ...
        const updateCount = () => {
          ...
          if (count < target) {
            ...
            requestAnimationFrame(updateCount); // Daha akıcı animasyon için
          } else {
        ...
        updateCount();
        observer.unobserve(counter); // Sadece bir kere say
      }
    });
  }, observerOptions);
